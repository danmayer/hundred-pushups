...
(defn app-root []
  (let [stage (subscribe [:stage])
        ui-state (subscribe [:ui-state/get])]
    (fn []
      (let [selected-tab (-> ui-state deref :tabs/selected)
            _ (prn "uistate" @ui-state)
            _ (prn "selected-tab" selected-tab)
            ]
        [tab-bar {}
         [tab-bar-item {:title "work out"
                        :selected (= :workout selected-tab)
                        :icon {:uri base64-icon :scale 3}
                        :on-press #(dispatch [:ui-state/set [:tabs/selected] :workout])}
          [view {:style {:flex-direction "column" :margin 40 :align-items "center"}}
         ...
         [tab-bar-item {:title "schedule"
                        :selected (= :schedule selected-tab)
                        :icon {:uri base64-icon :scale 3}
                        :on-press #(dispatch [:ui-state/set [:tabs/selected] :schedule])}
          [view {}
           [text {:style {:padding-top 100}} "<schedule stuff here>"]]]
         [tab-bar-item {:title "dev tools"
                        :selected (= :devtools selected-tab)
                        :icon {:uri base64-icon :scale 3}
                        :on-press #(dispatch [:ui-state/set [:tabs/selected] :devtools])}
          [view {}
           [text {:style {:padding-top 100}} "<devtools>"]]]]))))
